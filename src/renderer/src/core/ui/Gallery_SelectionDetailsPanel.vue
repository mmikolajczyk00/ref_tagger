...
<template>
  <div
    ...
    :class="[isActive ? 'max-w-full p-4' : 'p-1 max-w-12']"
  >
    <Button
      :class="[isActive ? 'absolute left-2 top-2 size-10' : 'size-full', ' ']"
      text
    ...
    <template v-if="isActive">
      <!-- file count -->
      <p class="border-surface-400 pb-4 border-b text-surface-300">
      ...
      <template v-if="selectedFiles.length > 0">
        ...
        <div
          class="bg-surface-800 flex-2 flex min-h-16 w-full flex-row flex-wrap content-start gap-1 overflow-y-auto rounded-none p-3"
        >
          <TagChip
