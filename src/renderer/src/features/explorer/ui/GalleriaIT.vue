...
<template>
  <div v-show="open">
    ...
    <div
      ...
      class="group absolute left-0 top-0 z-10 flex h-full w-48 flex-row justify-end overflow-y-auto overflow-x-hidden opacity-0 hover:opacity-100"
      ...
      >
        ...
        >
          <div
            ...
            class="bg-primary-500/25 z-5 absolute size-full border"
          ></div>
          <FileDisplay
    ...
    <div
      :class="[panning ? 'hidden' : '']"
      class="group absolute right-0 top-0 z-10 flex h-full w-48 flex-row justify-end"
    >
      <div
      ...
      >
        <div class="flex h-full flex-col items-center justify-center gap-0.5">
          <Button v-tooltip.left="'reset zoom'" class="rounded-none rounded-t" @click="zoom = 1"
    ...
    <span
      class="material-symbols-outlined text-5xl! z-15 bg-surface-950/50 absolute right-0 top-0 m-5 cursor-pointer rounded-full"
      @click.stop="close()"
    >
      close
    </span>
